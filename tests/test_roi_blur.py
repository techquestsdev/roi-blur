...
class TestBlurBoxes:
    ...
    def test_box_clamped_to_image_bounds(self, sample_image: np.ndarray):
        ...
        assert not np.array_equal(
            sample_image[40:100, 40:100],
            result[40:100, 40:100]
        )
...
class TestInteractiveRoiSelection:
    ...
    def test_multiple_selections_then_quit(self, sample_image: np.ndarray):
        ...
        with patch.object(cv2, 'imshow'), \
            ...
            mock_waitkey.side_effect = [1, ord(' '), 1, ord('q')]  # continue after first, quit after second
...
class TestSaveImage:
    ...
    def test_save_to_readonly_directory_raises(self, sample_image: np.ndarray, tmp_path: Path):
        ...
        # This test is platform-dependent; skip if we can't create read-only dir
...
class TestParseArgs:
    ...
    def test_custom_kernel_size(self, temp_image_file: Path, tmp_path: Path):
        ...
        output = tmp_path / "output.png"
        args = roi_blur.parse_args([
            str(temp_image_file), str(output),
            '--ksize', '51'
        ])
...
class TestCLIIntegration:
    ...
    def test_cli_basic_execution(self, temp_image_file: Path, tmp_path: Path):
        ...
        with patch.object(roi_blur, 'interactive_roi_selection') as mock_select, \
    ...
    def test_cli_no_rois_selected(self, temp_image_file: Path, tmp_path: Path):
        ...
        with patch.object(roi_blur, 'interactive_roi_selection') as mock_select, \
            ...
            try:
                ...
                pass  # Acceptable to exit when no ROIs selected

        # Output should still be created (with unmodified image)
        assert output_path.exists()

    def test_subprocess_execution(self, temp_image_file: Path, tmp_path: Path):
        """Test running the script as a subprocess."""
        output_path = tmp_path / "subprocess_output.png"

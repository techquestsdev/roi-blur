    ...
    while True:
        ...
        if key == ord("q"):
            ...
            if rois:
                ...
                print(
                    f"[UNDO] Removed ROI: x={removed[0]}, y={removed[1]}, "
...
def save_image(image: NDArray[np.uint8], output_path: Path) -> None:
    ...
    # Preflight to ensure output directory exists and is writable
    ...
    try:
        ...
        raise SystemExit(
            f"[ERROR] Output directory does not exist and cannot be created: {out_dir}\n        {e}"
        ) from e

    # Robust writability check (os.access is unreliable on Windows/ACLs)
    try:
